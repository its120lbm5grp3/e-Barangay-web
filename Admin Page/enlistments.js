...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const modalApprove =
    document.getElementById('approve-btn') || modal.querySelector('.approve-btn');
  const modalReject =
    document.getElementById('reject-btn') || modal.querySelector('.reject-btn');
    ...
    try {
      ...
      for (const docSnap of snapshot.docs) {
        ...
        viewBtn?.addEventListener('click', () =>
          openModal(docSnap.id, id - 1, data)
        );
        approveBtn?.addEventListener('click', () =>
          updateStatus(docSnap.id, 'approved')
        );
        rejectBtn?.addEventListener('click', () =>
          updateStatus(docSnap.id, 'denied')
        );
  ...
  function openModal(firebaseDocId, numericId, requestData) {
    ...
    let dateStr = '—';
    try {
      dateStr = requestData?.createdAt
        ? requestData.createdAt.toDate().toLocaleDateString('en-US', {
            month: 'numeric',
            day: 'numeric',
            year: 'numeric',
          })
